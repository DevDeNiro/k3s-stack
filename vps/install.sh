...
install_k3s() {
    ...
    local k3s_args="--disable traefik --disable servicelb --disable metrics-server"
...
setup_argocd_autodiscover() {
    ...
    # Apply the autodiscover ApplicationSet (without the secret - user must add token)
    if [[ -f "$SCRIPT_DIR/manifests/argocd-autodiscover.yaml" ]]; then
        ...
        echo -e "${YELLOW}⚠ Don't forget to set the SCM token (secure method):${NC}"
        echo -e "  ${BLUE}sudo ./vps/scripts/export-secrets.sh set-github-token${NC}"
    else
...
setup_postgres_backup() {
    ...
    if [[ -d "$K3D_STACK_ROOT/local/postgres-backup" ]]; then
        helm upgrade --install postgres-backup "$K3D_STACK_ROOT/local/postgres-backup" \
            --namespace storage \
            --wait --timeout 2m || true
        echo -e "${GREEN}✓ PostgreSQL backup CronJob configured${NC}"
    else
...
print_summary() {
    ...
    for ns in storage security monitoring ingress-nginx argocd; do
        echo -e "\n${BLUE}[$ns]${NC}"
