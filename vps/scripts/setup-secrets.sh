    ...
    case "$command" in
        ...
        rotate)
            ...
            if [[ "$confirm" =~ ^[Yy]$ ]]; then
                ...
                save_credentials
                create_k8s_secrets
                echo -e "\n${YELLOW}⚠️  Restart services to apply new passwords:${NC}"
                echo -e "  kubectl rollout restart statefulset/postgresql -n storage"
                echo -e "  kubectl rollout restart statefulset/keycloak -n security"
                echo -e "  kubectl rollout restart deployment/grafana -n monitoring"
            else
                echo "Aborted."
            fi
            ;;
